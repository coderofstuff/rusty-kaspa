...
impl PruningProofManager {
    ...
    // find_proof_and_consensus_common_ancestor_blue_work returns an option of a tuple
    // that contains the blue_work of the proof and current consensus common ancestor. If no
    // such ancestor exists, it returns None.
    fn find_proof_and_consensus_common_ancestor_blue_work(
        &self,
        ...
        let mut iteration = 2 * self.pruning_proof_m as usize;

        while !chain_block.direct_parents().is_empty() && !chain_block.hash.is_origin() && iteration > 0 {
            if ghostdag_stores[level as usize].has(chain_block.hash).unwrap() {
    ...
    pub fn validate_pruning_point_proof(&self, proof: &PruningPointProof) -> PruningImportResult<()> {
        ...
        for (level_idx, selected_tip) in selected_tip_by_level.iter().copied().enumerate() {
            ...
            // Step 1 - look at all levels that have a full proof (least 2m blocks in the proof)
            ...
            // old proof's tips and the common ancestor is less than the score difference between the proof's tip
            ...
            if let Some(common_ancestor) = self.find_proof_and_consensus_common_ancestor_blue_work(
                &ghostdag_stores,
