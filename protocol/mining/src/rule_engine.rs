...
pub struct MiningRuleEngine {
    ...
    is_sync_rate_below_threshold: Arc<AtomicBool>,
}
...
impl MiningRuleEngine {
    pub async fn worker(self: &Arc<MiningRuleEngine>) {
        ...
        loop {
            ...
            if elapsed_time.as_secs() > 0 {
                ...
                let rate: f64 = (delta.body_counts as f64) / (expected_blocks as f64);

                trace!(
                    "Sync rate: {:.2} | Elapsed time: {}s | Found/Expected blocks: {}/{}",
                    rate,
                ...
                if rate < SYNC_RATE_THRESHOLD {
                    if let Ok(false) =
                        self.is_sync_rate_below_threshold.compare_exchange(false, true, Ordering::Relaxed, Ordering::Relaxed)
                    {
                        warn!("Sync rate {:.2} is below threshold: {}", rate, SYNC_RATE_THRESHOLD);
                    }
                } else {
                    if let Ok(true) =
                        self.is_sync_rate_below_threshold.compare_exchange(true, false, Ordering::Relaxed, Ordering::Relaxed)
                    {
    ...
    pub fn new(flow_context: Arc<FlowContext>, processing_counters: Arc<ProcessingCounters>, tick_service: Arc<TickService>) -> Self {
        Self { flow_context, processing_counters, tick_service, is_sync_rate_below_threshold: Arc::new(AtomicBool::new(false)) }
    }
    ...
    pub async fn should_mine(&self, nearly_synced_finder: NearlySyncedFinder<'_>) -> bool {
        ...
        is_nearly_synced || self.is_sync_rate_below_threshold.load(std::sync::atomic::Ordering::Relaxed)
    }
